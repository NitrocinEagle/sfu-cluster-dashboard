    ...
    $.ajax({
        ...
        success: function (data) {
                    ...
                    if (node.header == info.node) {
                        node.data.forEach(function (plugin) {
                            if (plugin.header == info.plugin) {
                                plugin.data.push({
                                    header: info.param,
                                    graph: '<a href="/modules/monitoring_nodes/graphs/show/' + info.node + '/' + info.plugin + '/' + info.param + '"><i class="fa fa-2x fa-bar-chart"></i></a>',
            ...
            webix.ui({
                ...
                container: "treetable",
                columns: [
                    {id: "header", header: "Узлы, плагины, параметры", template: "{common.treetable()} #header#", width: 400},
