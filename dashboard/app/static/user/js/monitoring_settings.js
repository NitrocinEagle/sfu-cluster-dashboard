        ...
        if (fieldName == 'add_node') {
            ...
            if (fieldName == 'add_server_group') {
                ...
                if (fieldName == 'add_node_to_group' || fieldName == 'del_node_from_group') {
                    ...
                    if (fieldName == 'del_node_from_group') {
                        $select_group.on('change', function() {
                            $select_node.empty()
                            MONITORING_SETTINS_API.getNodesByGroup($select_group.val(), function (data) {
                                setOptions(data, $select_node);
                            });
                        })
                    } else {
                    ...
                    if (fieldName == 'stop_to_monitor_node') {
                        ...
                        if (fieldName == 'stop_to_monitor_param') {
                            ...
                            $select_node.on('change', function () {
                                $select_plugin.empty();
                                $select_param.empty();
                                MONITORING_SETTINS_API.getPluginsByNode($select_node.val(), function (data) {
                                    setOptions(data, $select_plugin);
                                });
                            });
                            $select_plugin.on('change', function () {
                                $select_param.empty();
                                MONITORING_SETTINS_API.getParamsByNodePlugin($select_node.val() + '/' + $select_plugin.val(), function (data) {
                                    setOptions(data, $select_param);
                                });
                            });
